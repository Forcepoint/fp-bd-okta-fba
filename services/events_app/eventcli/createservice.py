#
# functionality: class for creating okta_event.service systemd service
# Author:  Dlo Bagari
...
class CreateService:
    ...
    def _create_service(self, name):
        service = """
[Unit]
Description=Pulls events from Okta and send them to FBA
After=network.target network-online.target okta_user.service
Requires=okta_user.service

[Service]
ExecStart={}/scripts/event_service.sh events -c {}

[Install]
WantedBy=multi-user.target
""".format(self._config.APPLICATION_DIRECTORY, self.__args.config_file)
        service_path = "/etc/systemd/system/{}.service".format(name)
