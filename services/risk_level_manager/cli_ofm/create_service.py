#
# functionality: class for creating okta_risk_manager_service.service systemd service
# Author:  Dlo Bagari
...
class CreateService:
    ...
    def __call__(self, args):
        self.__args = args
        result, error = self._config.set_configs(self.__args.config_file)
        if result is False:
            self.__parser.error("Failed in loading config file")
    ...
    def _create_service(self, name):
        service = """
[Unit]
Description= Manage user's group membership
Wants=network-online.target okta_connector.service
After=network.target network-online.target okta_connector.service
Requires=okta_connector.service

[Service]
ExecStart={}/scripts/risk_level_manager_service.sh run -c {}
